...
main = launchAff_ $
  runSpec [ consoleReporter ] do
    ...
    describe "deleteVertex" do
      it "alterVertex will not delete key from parent's dependencies" do
        (Graph.toMap $ Graph.alterVertex (const Nothing) 4 acyclicGraph) `shouldNotEqual` (Graph.toMap $ acyclicGraphWithout4)
      it "works for an example" do
        (Graph.toMap $ Graph.deleteVertex 4 acyclicGraph) `shouldEqual` (Graph.toMap $ acyclicGraphWithout4)
